...
IQKeyLevelsGPU plots confluent price levels from multiple analysis layers on a single SharpDX-rendered overlay. Feature groups include: **session POCs** (Asia / London / NY volume profiles), **POC Cluster zones** (shaded high-confluence bands), **Session Gap Zones** (auto-detected daily and weekend gaps with partial-fill shrinking), **Session Opens/Closes**, **RD Range Daily**, **Psychological (Psy) Levels** (D/W/M), **Weekly/Monthly Hi-Lo**, **Hourly Opens**, and **Level 2 order-flow walls**. Cluster and gap zone widths auto-scale to each instrument via a `PercentOfADR` mode, self-adapting across NQ, ES, RTY, YM, CL, GC and their micros. Coincident-level labels from different feature layers merge automatically (e.g. `D+WPsy H 29962.50`), and right-anchored labels include a keep-out band around the live price box plus per-frame collision avoidance so labels never overprint.
