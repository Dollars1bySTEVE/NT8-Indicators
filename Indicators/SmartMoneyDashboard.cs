    ...
    public class SmartMoneyDashboard : Indicator
    {
        ...
        protected override void OnStateChange()
        {
            ...
            else if (State == State.DataLoaded)
            {
                ...
                // Now that DataLoaded has fired, create per-series indicators
                // Closes[bipIdx] is only safe to reference after data series are loaded
                foreach (var kvp in bipIndexMap)
                {
                    int bipIdx = kvp.Value;
        ...
        protected override void OnBarUpdate()
        {
            ...
            // For the primary series (BarsInProgress == 0), just trigger a chart repaint
            // so the dashboard overlay redraws with any updated data
            if (BarsInProgress == 0)
            {
                if (dataReady && IsFirstTickOfBar || Calculate == Calculate.OnEachTick)
                    ForceRefresh();
                return;
            }
            ...
            // Ensure enough bars — use CurrentBars[bip] which is the proper NT8 way
            int minBars = Math.Max(2, Math.Max(Math.Max(TrendEMAPeriod, TrendSMAPeriod), VolumeLookback));
            
            if (CurrentBars[bip] < minBars)
                return;
        ...
        {
            if (!EnableDashboard || chartControl == null || ChartBars == null)
                return;
            
            // Guard against null timeframeDataDict (OnRender can fire before DataLoaded)
            if (timeframeDataDict == null || timeframeDataDict.Count == 0)
                return;
        ...
        {
            if (!emaForBip.ContainsKey(bip) || CurrentBars[bip] < TrendEMAPeriod)
                return false;
        ...
        {
            if (!smaForBip.ContainsKey(bip) || CurrentBars[bip] < TrendSMAPeriod)
                return false;
        ...
        private bool CalculateVolumeTrend(int bip)
        {
            ...
            if (CurrentBars[bip] < VolumeLookback)
                return false;
        ...
        private void RenderDashboard(ChartControl chartControl, ChartScale chartScale)
        {
            ...
            // Safety check - ChartPanel must be available for positioning
            if (ChartPanel == null)
                return;
        ...
        {
            int count = (timeframeDataDict != null) ? timeframeDataDict.Count : 0;
            ...
            if (ShowTimeRemaining)
            {
                ...
                height += count * RowHeight; // Data rows
                height += 5; // Spacing
            }
        ...
        {
            // Use ChartPanel.X and ChartPanel.Y as the origin offset so our
            // coordinates are relative to the correct panel in multi-panel charts
            ...
            switch (PanelPosition)
            {
                ...
                case PanelPositionType.TopRight:
                    x = panelX + panelW - width - 10;
                    y = panelY + 10;
                    break;
                    
                case PanelPositionType.BottomLeft:
                    x = panelX + 10;
                    y = panelY + panelH - height - 10;
                    break;
