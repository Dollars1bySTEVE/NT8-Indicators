...
| Zone Label Background | `true` | Draw a subtle dark background chip behind **gap zone** and **POC cluster zone** labels. Zone label text remains full-opacity even when zone fill opacity is low or dimmed. |
...
- **Label Y-position collision system**: per-frame collision avoidance (`GetNonCollidingLabelY`) bucketed into two independent HashSets — one for left-anchored labels (`LineStart`), one for right-anchored (`LineEnd`) — cleared once per `OnRender` frame and shared across all render helpers. The vertical displacement step is at least `LabelFontSize + 4` pixels and at least the caller-provided label block height (used by zone label chips); collision detection uses that same occupied height to prevent near-touches/overlap. A keep-out band of `±LabelFontSize` pixels around the live chart price box (`_currentPriceY`) prevents labels from overprinting the current-price marker.
- **Zone labels (gap + cluster)** render with full-opacity text brushes independent of zone fill opacity. Optional background chips (`ZoneLabelBackground`) are sized from estimated text width plus small padding for readability over fills and candles.
